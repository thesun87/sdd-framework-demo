...
| Stock status | Tình trạng tồn kho | Whether a Product is sellable **right now**, derived from Stock at read time. Exactly two values: `in_stock` (Stock > 0) and `out_of_stock` (Stock = 0). It is the only stock-derived value a Guest or Customer ever sees — the Stock number itself never leaves the back office (AD-19), and this value is never cached (AD-20). The Vietnamese labels "Còn hàng"/"Hết hàng" are display-layer only | "availability", "stock level", "in stock flag", "tình trạng hàng", bare "còn hàng" as an identifier, any third value |
...
| Storefront | Trang bán hàng | The customer-facing surface where Guests and Customers browse, keep a cart and place orders. Never carries the Stock number (AD-19) | "shop front", "customer site", "frontend" as a surface name |
| Registration wall | Tường đăng ký | The deliberate stop between the cart and placing an order: a Guest must register or log in, then returns to the place-order step with the cart intact (FR-11). Never skippable — no guest checkout | "login wall", "paywall", "auth gate", "checkout gate" |
| Place order | Đặt đơn | The step where a Customer enters the delivery address, chooses a payment method and submits the order (FR-12–FR-15); also the act of submitting it | "checkout", "thanh toán" as the step name, "mua ngay" |
