...
- **Làm rõ 2026-09-23 (ADR-0001, `baseline-0002`):** "còn/hết" cho dòng giỏ được hiện thực bằng một **enum cấp dòng** `ok | exceeds_stock | out_of_stock` mà server tính từ cặp (sản phẩm, số lượng) — phép so sánh nằm trong `stock`, con số không rời module đó, và hợp đồng Trang bán hàng vẫn không có số nguyên tồn kho nào. **Rủi ro đã chấp nhận** (Tuan Nguyen): đổi số lượng và quan sát lúc trạng thái đổi cho phép suy ra con số chính xác sau ~log₂(trần số lượng) lần gọi; luật hình dạng giữ, mục đích "con số không rời Trang quản trị" chỉ còn giữ ở mức hợp đồng. Không có biện pháp giảm thiểu trong v1.
