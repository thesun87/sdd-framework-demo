...
- **FR-1…FR-33 đánh số toàn cục** và giữ nguyên số kể cả khi §4 được sắp xếp lại, để artifact hạ nguồn tham chiếu ổn định. Bảng tra FR theo tính năng ở §7.1. **Các FR trỏ ngược về** hành trình UJ-1…UJ-6 (§2.3) ở những chỗ mạch truy vết có ý nghĩa, không phải ở mọi FR.
- **`[ASSUMPTION]` là suy đoán của tài liệu này**, không phải điều discovery đã chốt. Danh sách đầy đủ ở §12.
- **Đây là bản nháp BMAD.** Một người sẽ curate nó thành `docs/baseline/prd.md` — bước A6 của protocol, mà protocol ghi rõ "Actor: Human. This step is not delegated to an agent" — rồi A7 mới đóng băng. Bản curate mới là baseline.

Nguồn đầu vào: `docs/discovery/README.md` (A0, chấp nhận 2026-09-18) và `../../briefs/brief-sdd-framework-demo-2026-09-18/brief.md`. Tài liệu này **xây trên** chúng, không lặp lại. Chiều sâu thuộc về tài liệu hạ nguồn nằm ở `addendum.md`.
...
| **Địa chỉ giao hàng** | Delivery address | Tên người nhận, số điện thoại, địa chỉ. Nhập cho từng đơn, sao chép vào đơn. Là dữ liệu cá nhân theo NĐ 13/2023/NĐ-CP. |
...
**Hệ quả kiểm chứng được:**
- Đăng ký chỉ yêu cầu định danh (số điện thoại hoặc email) và mật khẩu. `[ASSUMPTION: discovery không nêu định danh nào; số điện thoại là lựa chọn phổ biến ở Việt Nam và trùng với dữ liệu giao hàng vốn đã phải thu thập.]`
...
**Ba khác biệt so với ma trận trong discovery**, cả ba đều cố ý và đều đang chờ xác nhận ở §11:

1. **Huỷ đơn của chính mình** — dòng mới, discovery không bàn tới. → §11 Q5
2. **Chủ shop không thêm được vào giỏ hàng** — discovery cho ✓, nhưng chủ shop không đặt đơn được nên giỏ đó không có lối ra. → §11 Q12
3. **Chủ shop không đặt đơn được** — giữ nguyên như discovery. → §11 Q4
...
| 4.4 | Đăng ký, đăng nhập, tường đăng ký; tài khoản chủ shop tạo sẵn | FR-9 – FR-11, FR-33 |
...
| 4.10 | Back office: danh sách và chi tiết đơn | FR-29, FR-30 |
...
- **Nghị định 13/2023/NĐ-CP.** Tên, số điện thoại, địa chỉ giao hàng là dữ liệu cá nhân. Chỉ thu thập những gì việc giao hàng cần.
...
> `[ASSUMPTION]` Chính sách này do PRD đề xuất, không phải điều discovery đã chốt.
> Addendum §1 Q5 nêu ra căng thẳng nhưng không giải quyết. Chưa có tư vấn pháp lý.
> Cần xác nhận trước khi đóng băng baseline — xem §11 Q3.
...
### 11.1 Phải trả lời trước khi đóng băng baseline

Năm câu này quyết định mô hình dữ liệu hoặc ma trận phân quyền — hai thứ bị đóng băng. Trong Track A, đóng băng đứng trước `/speckit-specify`, nên không câu nào trong nhóm này hoãn sang phase sau được.

3. **Chính sách ẩn danh hoá.** Mốc 12 tháng ở §9.3 có được chấp nhận không? Cần đối chiếu với tư vấn pháp lý về NĐ 13/2023/NĐ-CP. Chặn vì nó quyết định mô hình dữ liệu của đơn hàng — thứ bị đóng băng. *(Carry-over: addendum §1 Q5, nay đã có đề xuất.)*

4. **Chủ shop đặt đơn hộ khách.** Nếu shop cần nhập đơn đặt qua điện thoại, đó là thay đổi phạm vi, không phải chi tiết hiện thực — và nó phá vỡ SM-2. §5 giữ nguyên "không". *(Carry-over: addendum §1 Q7.)*

5. **Quyền khách hàng tự huỷ đơn (FR-17).** Dòng mới trong ma trận phân quyền, không có trong discovery. Cần người quyết định xác nhận vì ma trận này bị đóng băng.

9. **Định danh đăng nhập.** Số điện thoại hay email? §4.4 giả định số điện thoại. Nó là khoá định danh của khách hàng — tức là thuộc mô hình dữ liệu bị đóng băng — và nó quyết định luôn Q10.

10. **Khôi phục mật khẩu.** Chưa có FR nào, và đó không phải sơ suất: không có email/SMS trong phạm vi thì đường khôi phục tự động **không tồn tại**. Ba phương án ở `addendum.md` §6, không cái nào hiển nhiên. Trong Track A, đóng băng baseline đứng trước `/speckit-specify`, nên câu này phải chốt ở bước freeze.
...
Mọi `[ASSUMPTION]` trong tài liệu, gom lại để xác nhận:
...
| §4.4 FR-9 | Định danh đăng ký là số điện thoại (hoặc email) — discovery không nêu. **[CHẶN]** |
